...
OUTPUT_DIR = ROOT / "assets" / "pinterest"
...
def title_layout(draw: ImageDraw.ImageDraw, title: str) -> tuple[object, list[str]]:
...
def render_article(article: dict, output: Path) -> None:
    ...
    kicker = str(article.get("pinterest_kicker") or (
        "DEALS & BUYING ADVICE" if any(word in lower_title for word in ("deal", "sale", "off")) else "UK BUYING GUIDE"
    ))
    subtitle = str(article.get("pinterest_subtitle") or "Practical shopping guidance for UK buyers")
    ...
    for label in labels:
        clean = label.strip()
        if not clean or clean.lower() in {"uk", "ebay", "buying guides"}:
            continue
    ...
    draw.text((70, 1385), "worthbuyinguk.blogspot.com", font=load_font(34, bold=True), fill=WHITE)
...
def main() -> None:
    ...
    for path in sorted(ARTICLES_DIR.glob("*.json")):
        ...
        count += 1
        print(f"[pinterest-image] {output.relative_to(ROOT)}")

    print(f"Generated {count} Pinterest image(s).")
